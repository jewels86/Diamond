...
public partial class SecureBigInteger
{
    ...
    public static SecureBigInteger GCD(SecureBigInteger a, SecureBigInteger b)
    {
        ...
        x = ParanoidRightShift(x, xTrailing, originalMaxBitLength);
        y = ParanoidRightShift(y, yTrailing, originalMaxBitLength);
    
        var maxIterations = (Math.Max(x.BitLength, y.BitLength) + 1) * 2;
    
        for (int i = 0; i < maxIterations; i++)
        {
            var maxBitLength = Math.Max(x.BitLength, y.BitLength);
            
            var xShifted = ParanoidRightShift(x, CountTrailingZeros(x), maxBitLength);
            x = Select(IsEven(x), xShifted, x);
    
            var yShifted = ParanoidRightShift(y, CountTrailingZeros(y), maxBitLength);
        ...
        var result = ParanoidLeftShift(x, commonShift, originalMaxBitLength);
